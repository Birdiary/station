...
def track_environment():
    try:
        ...
        environment = {"date": str(datetime.now()), "temperature": dht22.temperature, "humidity": dht22.humidity}
        ...
        try:
            ...
            if weight < weightThreshold:
                if len(values) >= 1:
                    ...
                    if environmentData is not None:
...
def send_movement(video_filename, audio_filename, data_filename, server_url, box_id):
    try:
        ...
        files = {'videoKey': (os.path.basename(video_filename), open(video_filename, 'rb')),
                 'audioKey': (os.path.basename(audio_filename), open(audio_filename, 'rb')),
